...
public class ContinueExtractor extends AbstractYuGongLifeCycle implements Runnable {
  ...
  private final Logger logger = LoggerFactory.getLogger(ContinueExtractor.class);

  public ContinueExtractor(
      OracleFullRecordExtractor oracleFullRecordExtractor,
  ...
  private Object getMinId() {
    ...
    Object min =
        jdbcTemplate.execute(
            oracleFullRecordExtractor.getGetMinPkSql(),
  ...
  public void run() {
    while (running) {
      jdbcTemplate.execute(
          oracleFullRecordExtractor.getExtractSql(),
          (PreparedStatementCallback)
              ps -> {
                ps.setObject(1, id);
                ...
                while (rs.next()) {
                  ...
                  for (ColumnMeta pk : context.getTableMeta().getPrimaryKeys()) {
                    ColumnValue cv =
                        oracleFullRecordExtractor.getColumnValue(
                            rs, context.getSourceEncoding(), pk);
                    pks.add(cv);
                  ...
                  for (ColumnMeta col : context.getTableMeta().getColumns()) {
                    ColumnValue cv =
                        oracleFullRecordExtractor.getColumnValue(
                            rs, context.getSourceEncoding(), col);
                    cms.add(cv);
                  }
