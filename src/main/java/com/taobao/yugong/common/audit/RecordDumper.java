...
public class RecordDumper {
  ...
  static {
    ...
    applier_format =
        "* batchId : [{0}] , extractorSize : [{1}] , applierSize : [{2}] , ackPosition [{3}] , Time : {4} "
            + SEP;
  ...
  public static void dumpExtractorInfo(
      Long batchId, List<Record> records, Position lastPosition, boolean dumpDetail) {
  ...
  public static void dumpApplierInfo(
      Long batchId,
      List<Record> extractorRecords,
      List<Record> applierRecords,
      Position position,
      boolean dumpDetail) {
  ...
  public static void extractorLog(String log) {
    Date now = new Date();
    SimpleDateFormat format = new SimpleDateFormat(TIMESTAMP_FORMAT);
    extractorLogger.info(log + "[current time]:" + format.format(now) + SEP);
  }

  public static void applierLog(String log) {
    Date now = new Date();
    SimpleDateFormat format = new SimpleDateFormat(TIMESTAMP_FORMAT);
    applierLogger.info(log + "[current time]:" + format.format(now) + SEP);
  }
}
