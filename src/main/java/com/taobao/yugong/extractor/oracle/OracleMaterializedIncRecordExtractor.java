...
public class OracleMaterializedIncRecordExtractor extends AbstractOracleRecordExtractor {
  ...
  private static final String MLOG_EXTRACT_FORMAT =
      "select * from (select rowid,{0} from {1}.{2} order by sequence$$ asc) where rownum <= ?";
  ...
  public void start() {
    ...
    String mlogTableName =
        TableMetaGenerator.getMLogTableName(context.getSourceDs(), schemaName, tableName);
    if (StringUtils.isEmpty(mlogTableName)) {
      throw new YuGongException("not found mlog table for [" + schemaName + "." + tableName + "]");
    }
    // 获取mlog表结构
    mlogMeta =
        TableMetaGenerator.getTableMeta(
            context.getSourceDs(), context.getTableMeta().getSchema(), mlogTableName);

    // 构造mlog sql
    String colstr = SqlTemplates.COMMON.makeColumn(mlogMeta.getColumns());
    mlogExtractSql =
        new MessageFormat(MLOG_EXTRACT_FORMAT)
            .format(new Object[] {colstr, schemaName, mlogTableName});
    mlogCleanSql =
        new MessageFormat(MLOG_CLEAN_FORMAT).format(new Object[] {schemaName, mlogTableName});
        ...
        new PreparedStatementCallback() {
          ...
          public Object doInPreparedStatement(PreparedStatement ps)
            ...
            for (Record record : records) {
              ...
              ps.setObject(
                  1, incRecord.getRowId().getValue(), incRecord.getRowId().getColumn().getType());
              ps.addBatch();
            }
  ...
  private List<OracleIncrementRecord> getMlogRecord(JdbcTemplate jdbcTemplate) {
    return (List<OracleIncrementRecord>)
        jdbcTemplate.execute(
            mlogExtractSql,
            new PreparedStatementCallback() {
              ...
              public Object doInPreparedStatement(PreparedStatement ps)
                ...
                while (rs.next()) {
                  ...
                  OracleIncrementRecord record =
                      new OracleIncrementRecord(
  ...
  private void buildMasterRecord(
      final JdbcTemplate jdbcTemplate, final List<OracleIncrementRecord> records) {
          ...
          template.submit(
              new Runnable() {
                ...
                public void run() {
                  ...
                  try {
                    MDC.put(
                        YuGongConstants.MDC_TABLE_SHIT_KEY, context.getTableMeta().getFullName());
  ...
  private void buildMasterRecordOneByOne(
      JdbcTemplate jdbcTemplate, final List<OracleIncrementRecord> records) {
    if (records.isEmpty()) {
      return;
    }
          ...
          public Object doInPreparedStatement(PreparedStatement ps)
            ...
            for (OracleIncrementRecord record : records) {
              ...
              try {
                ...
                if ((record.getOpType() == IncrementOpType.I
                        || record.getOpType() == IncrementOpType.U)
                    && !exist) {
    ...
    if (sqlUnit == null) {
      synchronized (this) {
        ...
        if (sqlUnit == null) { // double-check
          ...
          // 构造master sql
          String colstr =
              SqlTemplates.COMMON.makeColumn(context.getTableMeta().getColumnsWithPrimary());
          ...
          applierSql =
              new MessageFormat(MASTER_MULTI_PK_FORMAT)
                  .format(
                      new Object[] {colstr, record.getSchemaName(), record.getTableName(), priStr});
