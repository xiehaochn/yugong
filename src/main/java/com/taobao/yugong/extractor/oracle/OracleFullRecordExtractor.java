...
public class OracleFullRecordExtractor extends AbstractOracleRecordExtractor {
  ...
  @Getter private String extractSql;
  private Thread extractorThread = null;
  @Getter private String getMinPkSql;
  private final Logger logger = LoggerFactory.getLogger(OracleFullRecordExtractor.class);
  ...
  public void start() {
    ...
    if (StringUtils.isEmpty(extractSql)) {
      ...
      logger.info("colStr:" + colStr);
      this.extractSql =
          new MessageFormat(FORMAT)
              .format(new Object[] {colStr, schemaName, tableName, primaryKey});
    ...
    if (getMinPkSql == null && StringUtils.isNotBlank(primaryKey)) {
      this.getMinPkSql =
          new MessageFormat(MIN_PK_FORMAT).format(new Object[] {primaryKey, schemaName, tableName});
    }
