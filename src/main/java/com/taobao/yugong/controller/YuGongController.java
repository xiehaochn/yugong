...
import com.taobao.yugong.translator.KafkaTranslator;
...
public class YuGongController extends AbstractYuGongLifeCycle {
  ...
  public void start() {
    ...
    this.sourceDbType =
        DbType.valueOf(StringUtils.upperCase(config.getString("yugong.database.source.type")));
    this.targetDbType =
        DbType.valueOf(StringUtils.upperCase(config.getString("yugong.database.target.type")));
    ...
    for (TableHolder tableHolder : tableMetas) {
      Table currentTable = tableHolder.table;
      logger.info("init tableSchema:" + currentTable.getFullName());
      logger.info("primaryKeys:" + currentTable.getPrimaryKeys());
      logger.info("columns:" + currentTable.getColumns());
    }
    ...
    int noUpdateThresold =
        config.getInt("yugong.extractor.noupdate.thresold", noUpdateThresoldDefault);
    ...
    for (TableHolder tableHolder : tableMetas) {
      YuGongContext context =
          buildContext(globalContext, tableHolder.table, tableHolder.ignoreSchema);
      ...
      if (runMode == RunMode.KAFKA_INC || runMode == RunMode.KAFKA_ALL) {
        ...
        kafkaTranslator.setTable(tableHolder.table);
  ...
  private RecordExtractor chooseExtractor(
      TableHolder tableHolder,
      YuGongContext context,
      RunMode runMode,
      RecordPositioner positioner) {
    if (runMode == RunMode.KAFKA_INC) {
      runMode = RunMode.INC;
    } else if (runMode == RunMode.KAFKA_ALL) {
      runMode = RunMode.ALL;
    }
    ...
    if (runMode == RunMode.FULL || runMode == RunMode.CHECK) {
      ...
      if (StringUtils.isEmpty(extractSql)) {
        extractSql =
            config.getString(
                "yugong.extractor.sql." + fullName, config.getString("yugong.extractor.sql"));
      }
      ...
      if (forceFull
          || (isOnlyPkIsNumber(tableHolder.table)
              && !once
              && !tableOnce
        ...
        if (sourceDbType == DbType.ORACLE) {
          OracleOnceFullRecordExtractor recordExtractor =
              new OracleOnceFullRecordExtractor(context);
          recordExtractor.setExtractSql(extractSql);
          recordExtractor.setTracer(progressTracer);
          return recordExtractor;
        } else {
      ...
      if (sourceDbType == DbType.ORACLE) {
        ...
        recordExtractor.setConcurrent(
            config.getBoolean("yugong.extractor.concurrent.enable", true));
      ...
      AbstractRecordExtractor markExtractor =
          (AbstractRecordExtractor) chooseExtractor(tableHolder, context, RunMode.MARK, positioner);
      AbstractRecordExtractor fullExtractor =
          (AbstractRecordExtractor) chooseExtractor(tableHolder, context, RunMode.FULL, positioner);
      AbstractRecordExtractor incExtractor =
          (AbstractRecordExtractor) chooseExtractor(tableHolder, context, RunMode.INC, positioner);
  ...
  private RecordApplier chooseApplier(
    ...
    if (runMode == RunMode.KAFKA_INC || runMode == RunMode.KAFKA_ALL) {
      ...
      return new KafkaApplier(
          bootstrapServer, acks, retries, batchSize, lingerMs, bufferMemory, topic);
    }
    ...
    if (runMode == RunMode.FULL) {
      ...
      if (concurrent) {
        return new MultiThreadIncrementRecordApplier(
            context, threadSize, splitSize, applierExecutor);
      } else {
  ...
  private YuGongContext buildContext(
      YuGongContext globalContext, Table table, boolean ignoreSchema) {
    ...
    if (!isEmpty) {
      for (Object obj : tableWhiteList) {
        ...
        if (!isBlackTable(whiteTable, tableBlackList)) {
          ...
          if (strs.length == 1) {
            whiteTables =
                TableMetaGenerator.getTableMetasWithoutColumn(
                    globalContext.getSourceDs(), null, strs[0]);
            ignoreSchema = true;
          } else if (strs.length == 2) {
            whiteTables =
                TableMetaGenerator.getTableMetasWithoutColumn(
                    globalContext.getSourceDs(), strs[0], strs[1]);
          } else {
      ...
      List<Table> metas =
          TableMetaGenerator.getTableMetasWithoutColumn(globalContext.getSourceDs(), null, null);
  ...
  private DataTranslator buildExtKeys(Table table, String tableStr, DbType targetDbType) {
    ...
    if (targetDbType.isDRDS()) {
      ...
      String drdsExtKey =
          TableMetaGenerator.getShardKeyByDRDS(globalContext.getTargetDs(), schemaName, tableName);
