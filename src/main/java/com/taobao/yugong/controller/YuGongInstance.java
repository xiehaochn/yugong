...
public class YuGongInstance extends AbstractYuGongLifeCycle {
  ...
  public void start() {
    ...
    try {
      ...
      worker =
          new Thread(
              new Runnable() {
                ...
                public void run() {
                  try {
                    ...
                    for (int i = 0; i < retryTimes; i++) {
                      ...
                      if (i > 0) {
                        logger.info(
                            "table[{}] is start , retrying ", context.getTableMeta().getFullName());
                      } else {
                    ...
                    } else if (ExceptionUtils.getRootCause(exception)
                        instanceof InterruptedException) {
                      progressTracer.update(
                          context.getTableMeta().getFullName(), ProgressStatus.FAILED);
                      logger.info(
                          "table[{}] is interrpt ,current status:{} !",
                  ...
                  try {
                    ...
                    do {
                      ...
                      if (YuGongUtils.isNotEmpty(ackRecords)) {
                        statAggregation.push(
                            new AggregationItem(start, end, Long.valueOf(ackRecords.size())));
                      }
                    ...
                    logger.info(
                        "table[{}] is end by {}", context.getTableMeta().getFullName(), status);
                ...
                private List<Record> processTranslator(
                    final DataTranslator translator, List<Record> records) {
                  if (records.isEmpty()) {
                    return records;
                  }
                  ...
                  if (translator != null) {
                    if (translator instanceof BackTableDataTranslator) {
                      ...
                      records =
                          ((BackTableDataTranslator) translator)
                              .translator(
                                  context.getSourceDs(), context.getTargetDs(), records, template);
                    } else {
                ...
                private boolean processException(Throwable e, int i) {
                  if (!(ExceptionUtils.getRootCause(e) instanceof InterruptedException)) {
                    logger.error(
                        "retry {} ,something error happened. caused by {}",
                        (i + 1),
                        ExceptionUtils.getFullStackTrace(e));
                    try {
                      alarmService.sendAlarm(
                          new AlarmMessage(ExceptionUtils.getFullStackTrace(e), alarmReceiver));
                    } catch (Throwable e1) {
      ...
      logger.info(
          "table[{}] start successful. extractor:{} , applier:{}, translator:{}",
          new Object[] {
            context.getTableMeta().getFullName(),
  ...
  private void tpsControl(List<Record> result, long start, long end, long tps)
